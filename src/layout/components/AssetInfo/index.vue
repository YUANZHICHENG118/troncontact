  ...
  <div class="assetInfo">
    <asset-item>
      ...
      <ul>
        ...
        <!--个人级别  普通用户  VIP  超级VIP -->
        <li><span class="label">{{$t('account.userData.level')}}</span> <span>{{stats8===0?`${$t('account.userData.OrdinaryUser')}`:stats8===1?'VIP':`${$t('account.userData.super')}VIP`}}</span></li>
        <!--直推总人数-->
    ...
    <asset-item>
      ...
      <ul>
        ...
        <!--会员总提币-->
        <li><span class="label">{{$t('account.networkData.withdrawalTotal')}}</span> <span>{{g2}} TRX</span></li>
        <!--全球会员数-->
    ...
    <asset-item>
      ...
      <div class="copy">
        <el-button type="primary" v-clipboard:copy="address" v-clipboard:success="onCopy" v-clipboard:error="onError">
          {{$t('global.copy')}}
        </el-button>
      </div>
    </asset-item>
